...
module DOTIW
  class TimeHash
    ...
    private
      ...
      def build_seconds
        output[I18n.t(:seconds, :scope => options[:translation_scope], :default => "seconds")] = distance.to_i
        self.distance = 0
      end

      def build_minutes
        output[I18n.t(:minutes, :scope => options[:translation_scope], :default => "minutes")], self.distance = distance.divmod(1.minute)
      end

      def build_hours
        output[I18n.t(:hours, :scope => options[:translation_scope], :default => "hours")], self.distance = distance.divmod(1.hour)
      end

      def build_days
        output[I18n.t(:days, :scope => options[:translation_scope], :default => "days")], self.distance = distance.divmod(1.day)
      end

      def build_months
        build_years_months_days

        if (years = output.delete(I18n.t(:years, :scope => options[:translation_scope], :default => "years"))) > 0
          output[I18n.t(:months, :scope => options[:translation_scope], :default => "months")] += (years * 12)
        end
      end
      ...
      def build_years_months_days
        ...
        output[I18n.t(:years,  :scope => options[:translation_scope], :default => "years")]  = years
        output[I18n.t(:months, :scope => options[:translation_scope], :default => "months")] = months
        output[I18n.t(:days,   :scope => options[:translation_scope], :default => "days")]   = days
